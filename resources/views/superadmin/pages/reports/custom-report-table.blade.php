  ...
  {{-- {{$dates[$day]}} --}}
   {{-- @if(date('D', strtotime($data))=="Fri" || date('D', strtotime($data))=="Sat")
    ...
    <tr>
        ...
        <table class="table ">
           @php
           $total_cases=$forward->whereBetween('forward_date',[$dateFrom,$dateTo])->count();
           $acp_cases=$forward->whereBetween('forward_date',[$dateFrom,$dateTo])->where('accept_status',1)->where('forward_status',1)->count();
           $drop_cases=$forward->whereBetween('forward_date',[$dateFrom,$dateTo])->where('drop_status',1)->count();
           $rdata_relased=$release->whereBetween('release_date',[$dateFrom,$dateTo])->count();
           @endphp
           <tr>
        ...
        <th>
        {{$account->whereBetween('date',[$dateFrom,$dateTo])->sum('total')}}
        </th>
        <th >
        <table class="table ">
        @php
        $cash_total=$account->whereBetween('date',[$dateFrom,$dateTo])->where('payment_method','Cash')->sum('total');
        $mobile_total=$account->whereBetween('date',[$dateFrom,$dateTo])->where('payment_method','!=','Cash')->sum('total');
        @endphp
           <tr>
